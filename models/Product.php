...
class Product
{
     ...
     */
    public static function getLatestProducts($count = self::SHOW_BY_DEFAULT, $id = false)
    {
        // Соединение с БД
        $db = Db::getConnection();
        ...
        if ($id) {
            if ($id == 'name_a') {
                $sql .= ' ORDER BY name ASC LIMIT :count';
            }
            if ($id == 'name_d') {
                $sql .= ' ORDER BY name DESC LIMIT :count';
            }
            if ($id == 'price_a') {
                $sql .= ' ORDER BY price ASC LIMIT :count';
            }
            if ($id == 'price_d') {
                $sql .= ' ORDER BY price DESC LIMIT :count';
            }
        } else {
...
/**
     ...
     * @return type <p>Массив с товарами</p>
     */
    public static function getProductsListBySortCategory($categoryId, $id, $page)
    {
        if (strlen($page) > 1)
            $page = substr($page, 5);
        ...
        if ($id) {
            ...
            if ($id == 'price_d') {
                $sql .= ' ORDER BY price DESC LIMIT :limit OFFSET :offset';
            }
        } else {
